...
// Fetch Epics
const epicsJson = execSync('gh issue list --search "label:\\"type: epic\\" state:open" --json number,title,body,labels,state,assignees,url', { encoding: 'utf-8' });
const epics = JSON.parse(epicsJson);

// Fetch All Open Issues (limit 1000 to be safe)
const issuesJson = execSync('gh issue list --state open --limit 1000 --json number,title,body,labels,state,assignees,url', { encoding: 'utf-8' });
...
const getPriorityInfo = (issue) => {
  const pLabel = issue.labels.find(l => l.name.toLowerCase().startsWith('priority: '));
  if (!pLabel) {
    return { value: 4, label: 'None' };
  }
  const label = pLabel.name.split(': ')[1] || 'None';
  switch (label.toUpperCase()) {
    case 'P0': return { value: 0, label };
    case 'P1': return { value: 1, label };
    case 'P2': return { value: 2, label };
    case 'P3': return { value: 3, label };
    default: return { value: 4, label: 'None' };
  }
};
...
const getAssignee = (issue) => {
  return issue.assignees && issue.assignees.length > 0 ? issue.assignees.map(a => a.login).join(', ') : '';
};
